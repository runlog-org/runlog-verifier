...
// skipIfPostgresUnavailable surfaces a t.Skip if any of the postgres
// reexecute driver's runtime prereqs is missing — psql on PATH,
// RUNLOG_VERIFY_PGURL set, server reachable. Used by the canonical-seed
// tests so each one stays focused on its seed-specific assertions.
...
// TestVerifyCanonicalSeedUpdateReturningPlanningTime runs the canonical
// `postgres-update-returning-with-large-in-list-is-quadratic-use-values-join.yaml`
// seed end-to-end through verify.Run and asserts status:"verified". This is
// the canonical-seed exerciser for the F79 plan_node planning-time
// differential shape — the seed declares
// failed_branch_planning_time_seconds_gt: 10 and
// working_branch_planning_time_seconds_lt: 1, gating the IN-list-quadratic
// pattern (failed branch slow at planning, working branch fast).
//
// Skip conditions are identical to TestVerifyCanonicalSeedJsonbContainment.
func TestVerifyCanonicalSeedUpdateReturningPlanningTime(t *testing.T) {
	seedPath := filepath.Join(
		"..", "..", "..", "runlog", "server", "seeds",
		"postgres-update-returning-with-large-in-list-is-quadratic-use-values-join.yaml",
	)
	...
	if !strings.Contains(res.UnitID, "postgres-update-returning-with-large-in-list-is-quadratic-use-values-join") {
		t.Fatalf("UnitID=%q, does not contain canonical seed unit_id", res.UnitID)
	}
}
